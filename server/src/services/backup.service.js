...
class BackupService {
    ...
    getBackupUri() {
        ...
        const backupDbName = process.env.BACKUP_DB_NAME || process.env.DB_NAME + "_backup";
    ...
    async connectToBackup() {
        try {
            ...
            if (!backupUri) {
                throw new Error("MONGODB_BACKUP_URI is not defined in environment variables");
            }

            // Always create a fresh connection
            const connection = await mongoose.createConnection(backupUri, {
                maxPoolSize: 10,
    ...
    async syncCollection(collectionName, backupDb) {
        try {
            ...
            const result = await backupCollection.insertMany(documents, { ordered: false });
    ...
    async performBackup() {
        ...
        try {
            ...
            if (backupResult.success) {
                ...
                console.log(`✓ Database backup completed (${totalDocuments} documents in ${duration}ms)`);
            } else {
    ...
    getStatus() {
        return {
            ...
            backupUri: this.isConnected ? this.getBackupUri().replace(/\/\/([^:]+):([^@]+)@/, '//$1:****@') : null,
            stats: this.stats,
        };
    }
